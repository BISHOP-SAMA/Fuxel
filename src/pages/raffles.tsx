...
const mockRaffles = [
  { id: 1, title: "Exclusive Fox NFT Drop", description: "Enter for a chance to win an exclusive 1-of-1 Tempo Fox NFT from the genesis collection", prize: "1-of-1 Genesis Fox NFT", entryCost: 50, currentEntries: 47, maxEntries: 200, endsAt: new Date(Date.now() + 7 * 86400000).toISOString(), status: "active", winnerAddress: null },
  { id: 2, title: "$1000 pathUSD Jackpot", description: "The biggest FUXEL raffle yet — $1000 pathUSD up for grabs for the lucky winner", prize: "$1000 pathUSD", entryCost: 100, currentEntries: 213, maxEntries: 500, endsAt: new Date(Date.now() + 3 * 86400000).toISOString(), status: "active", winnerAddress: null },
];

const pastRaffles = [
  { id: 3, title: "Alpha Pass Giveaway", description: "Alpha Access Pass", prize: "Alpha Access Pass", entryCost: 25, currentEntries: 100, maxEntries: 100, endsAt: new Date(Date.now() - 86400000).toISOString(), status: "ended", winnerAddress: "0x1234...5678" },
];
  ...
  return (
    <Shell>
      <div className="space-y-8">
        ...
        <h2 className="text-xl font-bold text-amber-500 uppercase tracking-widest border-b border-amber-500/20 pb-2">Active Vaults</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {mockRaffles.map((raffle) => (
            <Card key={raffle.id} className="bg-black/40 border-amber-500/30 backdrop-blur-md flex flex-col relative overflow-hidden">
              ...
              <CardContent className="flex-1 space-y-4">
                ...
                <div className="w-full bg-black/50 rounded-full h-1.5">
                  <div className="bg-amber-500 h-1.5 rounded-full" style={{ width: `${Math.min(100, (raffle.currentEntries / raffle.maxEntries) * 100)}%` }} />
                </div>
              </CardContent>
              <CardFooter className="pt-4 border-t border-white/5">
                <Button onClick={() => handleEnter(raffle.id)} disabled={!address || entering === raffle.id} className="w-full bg-amber-500 text-black hover:bg-amber-600 font-bold uppercase tracking-wider">
                  <Ticket className="mr-2 h-4 w-4" />
                  {entering === raffle.id ? "Processing..." : "Secure Entry"}
                </Button>
              </CardFooter>
            </Card>
          ))}
